...
namespace DataTierGenerator {
	...
	/// </summary>
	public sealed class OracleGenerator {
		...
		public static void CreateInsertStoredProcedure(Table table, string grantLoginName, string storedProcedurePrefix, string path, bool createMultipleFiles) {
            ...
            string procedureName = storedProcedurePrefix + (table.Name.Substring(3, 1) == "_" ? table.Name.Substring(0, 3) + "S" + table.Name.Substring(3, table.Name.Length - 3) : table.Name.Substring(0, 3) + "S" + table.Name.Substring(4, table.Name.Length - 4)) + "_Insertar";
			string fileName;
			...
			using (StreamWriter writer = new StreamWriter(fileName, true)) {
				...
				writer.WriteLine("PROCEDURE " + procedureName + " (");

				// Create the parameter list
				for (int i = 0; i < table.Columns.Count; i++) {
					Column column = (Column) table.Columns[i];
					if (i < (table.Columns.Count - 1)) {
						writer.WriteLine("\t" + Utility.CreateOracleParameterString(column, true) + ",");
					} else {
                        writer.WriteLine("\t" + Utility.CreateOracleParameterString(column, true));
					}
				}
				...
				writer.WriteLine("AS");                
                writer.WriteLine("/*********************************");
                writer.WriteLine("*Descripcion : Inserta un Registro de la Tabla " + table.Name);
                writer.WriteLine("*Fecha Crea  : " + DateTime.Now.ToShortDateString());
                writer.WriteLine("*Fecha Mod   : ");
				...
				for (int i = 0; i < table.Columns.Count; i++) {
					...
					if (column.IsIdentity == false) {
						...
						if (i < (table.Columns.Count - 1)) {
							writer.WriteLine("\tpi_" + column.Name + ",");
						} else {
							writer.WriteLine("\tpi_" + column.Name);
						}
					}
                ...
                writer.WriteLine("END " + procedureName + ";");
			...
			if (table.PrimaryKeys.Count > 0 && table.Columns.Count != table.PrimaryKeys.Count && table.Columns.Count != table.ForeignKeys.Count) {
				...
				using (StreamWriter writer = new StreamWriter(fileName, true)) {
					...
					for (int i = 0; i < table.Columns.Count; i++) {
						...
						if (i < (table.Columns.Count  - 1)) {
							writer.WriteLine("\t" + Utility.CreateParameterString(column, false,0) + ",");
						} else {
							writer.WriteLine("\t" + Utility.CreateParameterString(column, false,0));
						}
					}
					...
					writer.WriteLine("AS");
                    writer.WriteLine("/*********************************");
                    writer.WriteLine("*Descripcion : Actualiza un Registro de la Tabla " + table.Name);
                    writer.WriteLine("*Fecha Crea  : " + DateTime.Now.ToShortDateString());
                    writer.WriteLine("*Fecha Mod   : ");
                    writer.WriteLine("*Parametros  : ");
                    for (int i = 0; i < table.Columns.Count; i++)
		...
		public static void CreateDeleteStoredProcedure(Table table, string grantLoginName, string storedProcedurePrefix, string path, bool createMultipleFiles) {
			if (table.PrimaryKeys.Count > 0) {
				...
				using (StreamWriter writer = new StreamWriter(fileName, true)) {
					...
					writer.WriteLine("AS");
                    writer.WriteLine("/*********************************");
                    writer.WriteLine("*Descripcion : Elimina un Registro de la Tabla "+ table.Name);
                    writer.WriteLine("*Fecha Crea  : " + DateTime.Now.ToShortDateString());
                    writer.WriteLine("*Fecha Mod   : " );
                    writer.WriteLine("*Parametros  : ");
                    for (int i = 0; i < table.PrimaryKeys.Count; i++)
		...
		public static void CreateSelectStoredProcedure(Table table, string grantLoginName, string storedProcedurePrefix, string path, bool createMultipleFiles) {
			if (table.PrimaryKeys.Count > 0 && table.ForeignKeys.Count != table.Columns.Count) {
				...
				using (StreamWriter writer = new StreamWriter(fileName, true)) {
					...
					writer.WriteLine("AS");
                    writer.WriteLine("/*********************************");
                    writer.WriteLine("*Descripcion : Selecciona un Registro de la Tabla " + table.Name);
                    writer.WriteLine("*Fecha Crea  : " + DateTime.Now.ToShortDateString());
                    writer.WriteLine("*Fecha Mod   : ");
                    writer.WriteLine("*Parametros  : ");
                    for (int i = 0; i < table.PrimaryKeys.Count; i++)
		...
		public static void CreateSelectAllStoredProcedure(Table table, string grantLoginName, string storedProcedurePrefix, string path, bool createMultipleFiles) {
			if (table.PrimaryKeys.Count > 0 && table.ForeignKeys.Count != table.Columns.Count) {
				...
				using (StreamWriter writer = new StreamWriter(fileName, true)) {
					...
					writer.WriteLine("AS");
                    writer.WriteLine("/*********************************");
                    writer.WriteLine("*Descripcion : Lista Todos los Registros de la Tabla " + table.Name);
                    writer.WriteLine("*Fecha Crea  : " + DateTime.Now.ToShortDateString());
                    writer.WriteLine("*Fecha Mod   : ");                    
                    writer.WriteLine("*Autor       : ");
                    writer.WriteLine("*********************************/");   
